...
system("plink --tfile temp5_${name[0]} --make-bed --out $name[0].flip");
...
if (-s "monomorphic.txt"){
	...
	system("plink --bfile $name[0].flip --exclude monomorphic.txt --make-bed --out $name[0].flip_mono");
	system("plink --bfile $name[0].flip_mono --exclude multiallelic_SNPs.txt --make-bed --out $name[0].flip_mono_nomulti");
	...
	system("plink --bfile 1KG_Omni --extract common_snps_1KG.txt --make-bed --out 1KG_Omni_overlap");
	system("plink --bfile $name[0].flip_mono_nomulti --extract common_snps_1KG.txt --make-bed --out $name[0].flip_mono_nomulti_overlap");
	system("plink --bfile 1KG_Omni_overlap --bmerge $name[0].flip_mono_nomulti_overlap.bed $name[0].flip_mono_nomulti_overlap.bim $name[0].flip_mono_nomulti_overlap.fam --recode --out merged_data");

	if(-e "merged_data.missnp"){
		system("plink --bfile 1KG_Omni_overlap --exclude merged_data.missnp --make-bed --out 1KG_Omni_overlap_nomiss");
		system("plink --bfile $name[0].flip_mono_nomulti_overlap --exclude merged_data.missnp --make-bed --out $name[0].flip_mono_nomulti_overlap_nomiss");
		system("plink --bfile 1KG_Omni_overlap --bmerge $name[0].flip_mono_nomulti_overlap_nomiss.bed $name[0].flip_mono_nomulti_overlap_nomiss.bim $name[0].flip_mono_nomulti_overlap_nomiss.fam --recode --out merged_data_2");
		
		print "\n\nSi no hay un archivo merged_data_2.bed; entonces hay un problema!!!\n\n";

		####Find controls in merged_data
		@controls_1KG=(HG01946,HG01935,NA21440,NA21317);
		%ids_uniq=();

		open(PED,"merged_data_2.fam");
		while(<PED>){
			chomp;
			@split2=split(/\t/,$_);
			for $id (@controls_1KG){
		...
		system("sed -nr '/${control}/p' merged_data_2.ped > merged_data_3.ped");
		system("cp merged_data_2.map merged_data_3.map");
		system("plink --file merged_data_3 --distance allele-ct square ");

		$difference=`cut -f1 plink.dist | tail -1`;
		chomp($difference);
		$total=`wc -l merged_data_3.map | cut -d' ' -f1`;
		chomp($total);
		...
		system("plink --file merged_data_2 --distance allele-ct square ");
	...
	system("plink --bfile $name[0].flip --exclude multiallelic_SNPs.txt --make-bed --out $name[0].flip_nomulti");
	...
	###Make the merge with plink 
	system("plink --bfile 1KG_Omni --extract common_snps_1KG.txt --make-bed --out 1KG_Omni_overlap");
	system("plink --bfile $name[0].flip_nomulti --extract common_snps_1KG.txt --make-bed --out $name[0].flip_nomulti_overlap");
	system("plink --bfile 1KG_Omni_overlap --bmerge $name[0].flip_nomulti_overlap.bed $name[0].flip_nomulti_overlap.bim $name[0].flip_nomulti_overlap.fam --recode --out merged_data");
...
system("rm 1KG_Omni_overlap_nomiss.*");
